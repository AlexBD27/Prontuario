<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h2 class="text-2xl font-bold mb-4">Reestablecer Sistema</h2>

                    <form id="tramiteForm" action="{{ route('prontuario.resetAll') }}" method="POST">
                        @csrf
    ...
    <script>
        document.getElementById('resetbtn').addEventListener('click', function (event) {
            ...
            Swal.fire({
                title: '¿Estás seguro de querer reestablecer el sistema por completo?',
                text: "Recuerda que se borrarán todos los registros de la base de datos y los archivos almacenados",
                icon: 'warning',
    ...
    <script>
        @if(session('success'))
            Swal.fire({
                title: 'HECHO',
                text: 'Se ha reestablecido el sistema. Ten en cuenta que las credenciales de admin son por defecto',
                icon: 'success',
