        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h2 class="text-2xl font-bold text-gray-800 mb-6 border-b pb-2">Generar Nuevo Número Público</h2>
                    @if ($errors->has('error'))
                        <script>
                        ...
                        <div class="space-y-4">
                            ...
                            <div id="camposPublico" class="space-y-4 px-3">
                                <div>
                                    ...
                                    <select name="tipo_publico" id="tipo_publico" class="select2-documento w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 transition duration-150 ease-in-out">
                                        <option value="" disabled selected>Seleccione un tipo de público</option>
                                        @foreach($public_types as $publictype)
                                            <option value="{{ $publictype->id }}">{{ $publictype->description }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
