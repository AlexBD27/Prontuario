    ...
    <div class="py-12">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-end mb-6">
                <a href="{{ route('grouptype.create') }}" class="bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-6 rounded transition duration-300 ease-in-out flex items-center">
            ...
            <div class="overflow-x-auto bg-white rounded-lg shadow w-full p-6">
                <table id="grouptype-table" class="min-w-full leading-normal w-full">
                    <thead>
                        <tr>
                    ...
                    <tbody id="grouptypes-body">
                        ...
                        <tr class="odd:bg-gray-50 even:bg-white">
                            ...
                            <td class="px-3 py-3 border-b border-gray-200 bg-white text-sm text-center">
                                {{ $group_type->abbreviation }}
                            </td>
                            <td class="px-3 py-3 border-b border-gray-200 bg-white text-sm text-center">
                                {{ Str::limit($group_type->description, 50) }}
                            </td>
                            <td class="px-3 py-3 border-b border-gray-200 bg-white text-sm text-center">
                                <span class="relative inline-block px-3 py-1 font-semibold {{ $group_type->active ? 'text-green-900' : 'text-red-900' }} leading-tight">
                                    <span aria-hidden class="absolute inset-0 {{ $group_type->active ? 'bg-green-200' : 'bg-red-200' }} opacity-50 rounded-full"></span>
                                    <span class="relative">{{ $group_type->active ? 'Activo' : 'Inactivo' }}</span>
                                </span>
                            </td>
                            <td class="px-3 py-3 border-b border-gray-200 bg-white text-sm text-center">
                                <div class="flex justify-center items-center space-x-2">
                                    <a href="{{ route('grouptype.edit', $group_type->id) }}" class="text-blue-600 hover:text-blue-900">
                                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                        </svg>
                                    </a>
                                    <form action="{{ route('grouptype.destroy', $group_type->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que quieres eliminar este Tipo de Grupo?');">
                                        @csrf
                                        @method('DELETE')
    ...
    @push('scripts')
        ...
        <script>
            $(document).ready(function () {
                const table = $('#grouptype-table').DataTable({
                    responsive: true,
                    language: {
                ...
                const searchContainer = $('#grouptype-table_filter');
                const lengthContainer = $('#grouptype-table_length');
